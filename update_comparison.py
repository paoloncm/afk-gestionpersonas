...
new_hub = '''        <div class="hud-main">
          ...
          <section id="starkDecisionHub" class="stark-card" style="margin-bottom:24px; display:grid; grid-template-columns: 2fr 1fr 1fr; gap:24px; border:2px solid var(--accent); background:rgba(34,213,238,0.05);">
            ...
            <div style="border-right:1px solid rgba(255,255,255,0.1); padding-right:20px;">
              ...
              <div id="winnerReason" class="soft" style="font-size:14px; line-height:1.6; min-height:60px;">Calculando matriz de decisión Stark Industries...</div>
...
start_tag = '<div class="hud-main">'
end_tag = '<section class="stark-card" style="margin-bottom:40px;">'

# Find the start of hud-main
idx_start = content.find(start_tag)
# Find the start of Matrix section
idx_end = content.find(end_tag)

if idx_start != -1 and idx_end != -1:
    new_content = content[:idx_start] + new_hub + content[idx_end:]
    
    # Also replace Analysis Header
    new_content = new_content.replace('<h3>Análisis Detallado</h3>', '<h3 style="margin:40px 0 24px; font-family:\'Outfit\', sans-serif; font-weight:900; text-transform:uppercase; letter-spacing:2px; font-size:20px; opacity:0.8;">Análisis Detallado Stark</h3>')
    
    with open('comparison.html', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("SUCCESS: comparison.html updated.")
else:
    print(f"ERROR: Tags not found. Start: {idx_start}, End: {idx_end}")
