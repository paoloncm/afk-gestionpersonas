...
new_content = r'''        <div class="hud-main">
          ...
          <section id="starkDecisionHub" class="stark-card" style="margin-bottom:24px; display:grid; grid-template-columns: 2fr 1fr 1fr; gap:24px; border:2px solid var(--accent); background:rgba(34,213,238,0.05);">
            ...
            <div style="border-right:1px solid rgba(255,255,255,0.1); padding-right:20px;">
              ...
              <div id="winnerStats" style="display:flex; gap:10px; margin-bottom:12px;">
                 <span id="winnerConfidence" class="badge">CONFIANZA: --%</span>
                 <span id="winnerMatch" class="badge">MATCH: --%</span>
              </div>
...
with open('comparison.html', 'r', encoding='utf-8', errors='ignore') as f:
    content = f.read()

# Pattern captures from the first stark-card in hud-main to the Details header
pattern = r'<div class="hud-main">.*?Análisis Detallado'
# Replacement including the matched start and end tag to ensure we don't break the HTML
output = re.sub(pattern, new_content, content, flags=re.DOTALL)

with open('comparison.html', 'w', encoding='utf-8') as f:
    f.write(output)
print("SUCCESS: HTML update completed via Python script.")
