...
async function run() {
    const { data: tables, error } = await supabase.from('workers').select('*').limit(1);
    console.log("WORKERS FOUND:", !!tables);
    const { data: creds, error: e2 } = await supabase.from('worker_credentials').select('*').limit(1);
    console.log("CREDENTIALS FOUND:", !!creds, e2?.message);
}
run();
